...
class Expr:
    ...
    def _make_logical_inverse(self):
        if self == TRUE: return FALSE
        elif self == FALSE: return TRUE
        elif self.op == '~':  return self.args[0]
        else: return Expr('~',  self)
    ...
    def __invert__(self):        return self._make_logical_inverse()
...
def pl_resolution(KB, alpha):
    ...
    if type(alpha) is str: alpha = expr(alpha)
    clauses = KB.clauses + conjuncts(to_cnf(~alpha))
    derived = set()
    added = [] + clauses
    added = set(added)
    allAdded = [] + clauses
    allAdded = set(allAdded)
    newClause = set()
    print "Initial clause set:"
    for c in clauses: 
        print "  ", c
        if (len(c.args) == 0) or (len(c.args) == 1):
            derived.add(c)

    if FALSE in clauses: return True
    res_count = 0
    round = 0
    while True:
        temp = [] + clauses
        ...
        for clause in temp:
            for literal in derived:
                tempSen = (literal >> clause)
                tempSen = to_cnf(tempSen)
                if (tempSen == TRUE) and (len(clause.args) > 1):
                    clauses.remove(clause)
                    break
        
        n = len(clauses)
        nNew = len(newClause)
        newClauseList = list(newClause)
        temp = [] + newClauseList
        for clause in temp:
            for literal in derived:
                tempSen = (literal >> clause)
        ...
        if (round == 1):
            ...
            pairs = [(clauses[i], newClauseList[j])
                    for i in range(n) for j in range(nNew)]
        for (ci, cj) in pairs:
            resolvents = pl_resolve(ci, cj)
            res_count += 1
            if resolvents != []:
                print ('[%3d] ' % n), "Resolving",ci,"with",cj," : ",
                tempRes = [] + resolvents
                for res in tempRes: 
                    print res
                    if (len(res.args) == 0) or (len(res.args) == 1):
                ...
                if FALSE in resolvents:
                    print res_count, "calls to pl_resolve before contradiction found"
                    return True
...
def pl_nature(expr):
    kb = PropKB()
    result = pl_resolution(kb, expr)
    if (result == True):
        return "tautology"
    result2 = pl_resolution(kb, ~expr)
    if (result2 == True):
